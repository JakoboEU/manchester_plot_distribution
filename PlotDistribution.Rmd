...
woodland_and_scrub_no_ss_richness$plot_type = 'Distributed, no sites'
```
...
grassland_no_ss_richness$plot_type = 'Distributed, no sites'
```
...
built_no_ss_richness$plot_type = 'Distributed, no sites'
```
...
green_no_ss_richness$site_type = 'Not Build Surface'
green_no_ss_richness$plot_type = 'Distributed, no sites'
```
...
ggplot(distributed_sites, aes(x = site_type, y = bird_richness, color = plot_type)) + geom_boxplot() + bird_ylim + 
  scale_x_discrete(guide = guide_axis(n.dodge = 3)) + theme(legend.position = 'bottom')
```

```{r}
kruskal.test(distributed_sites$bird_richness ~ distributed_sites$site_type)
pairwise.wilcox.test(distributed_sites$bird_richness, distributed_sites$site_type)
```
...
```{r}
kruskal.test(distributed_sites$insect_richness ~ distributed_sites$site_type)
pairwise.wilcox.test(distributed_sites$insect_richness, distributed_sites$site_type)
```
...
```{r}
kruskal.test(distributed_sites$plant_richness ~ distributed_sites$site_type)
pairwise.wilcox.test(distributed_sites$plant_richness, distributed_sites$site_type)
```
