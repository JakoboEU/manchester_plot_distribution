...
```{r}
habitat_types_no_single_sites = habitat_types %>% filter(!(title %in% gm_gs$title)) %>% filter(!(title %in% gm_sbi$title))
habitat_types_no_single_sites
```

```{r}
PLOT_TYPE_DIST_NO_SINGLE_SITES = 'Distributed,\nno single sites'
PLOT_TYPE_DIST = 'Distributed'
PLOT_TYPE_SINGLE_SITE = 'Single Site'
```
...
woodland_and_scrub_richness$plot_type = PLOT_TYPE_DIST

woodland_and_scrub_no_ss_richness = sampled_richness_df(
  habitat_types_no_single_sites %>% filter(plant_insect_habitat_type == 'Woodland and Scrub'),
  habitat_types_no_single_sites %>% filter(bird_habitat_type == 'Woodland and Scrub')
)
woodland_and_scrub_no_ss_richness$site_type = 'Woodland and Scrub'
woodland_and_scrub_no_ss_richness$plot_type = PLOT_TYPE_DIST_NO_SINGLE_SITES
```
...
grassland_richness$plot_type = PLOT_TYPE_DIST

grassland_no_ss_richness = sampled_richness_df(
  habitat_types_no_single_sites %>% filter(plant_insect_habitat_type == 'Amenity Grassland'),
  habitat_types_no_single_sites %>% filter(bird_habitat_type == 'Amenity Grassland')
)
grassland_no_ss_richness$site_type = 'Amenity Grassland'
grassland_no_ss_richness$plot_type = PLOT_TYPE_DIST_NO_SINGLE_SITES
```
...
built_richness$plot_type = PLOT_TYPE_DIST

built_no_ss_richness = sampled_richness_df(
  habitat_types_no_single_sites %>% filter(plant_insect_habitat_type == 'Built Surface'),
  habitat_types_no_single_sites %>% filter(bird_habitat_type == 'Built Surface')
)
built_no_ss_richness$site_type = 'Built Surface'
built_no_ss_richness$plot_type = PLOT_TYPE_DIST_NO_SINGLE_SITES
```
...
green_richness$site_type = 'Not Built Surface'
green_richness$plot_type = PLOT_TYPE_DIST

green_no_ss_richness = sampled_richness_df(
  habitat_types_no_single_sites %>% filter(plant_insect_habitat_type %in% c('Woodland and Scrub', 'Amenity Grassland')),
  habitat_types_no_single_sites %>% filter(bird_habitat_type %in% c('Woodland and Scrub', 'Amenity Grassland'))
)
green_no_ss_richness$site_type = 'Not Built Surface'
green_no_ss_richness$plot_type = PLOT_TYPE_DIST_NO_SINGLE_SITES
```

```{r}
all_sites$plot_type = PLOT_TYPE_SINGLE_SITE
distributed_sites = bind_rows(
  woodland_and_scrub_richness, grassland_richness, built_richness, green_richness,
  woodland_and_scrub_no_ss_richness, grassland_no_ss_richness, built_no_ss_richness, green_no_ss_richness,
  all_sites)

distributed_sites$site_type <- factor(distributed_sites$site_type, levels = c(
  "Woodland and Scrub", "Amenity Grassland", "Not Built Surface", "Built Surface",
  "SBI", "Public Park Or Garden", "Cemetery", "Golf Course", "Playing Field", "Other Sports Facility"
  ))

distributed_sites_for_kruskal = distributed_sites[distributed_sites$plot_type != PLOT_TYPE_DIST,]
```
...
```{r}
scale_color_manual = scale_color_manual(values = setNames(
  c(COLOUR_2, COLOUR_6, COLOUR_8),
  c(PLOT_TYPE_DIST_NO_SINGLE_SITES, PLOT_TYPE_DIST, PLOT_TYPE_SINGLE_SITE))
)

theme_common = function(gg, taxa) { gg + 
    theme_bw() + 
    scale_x_discrete(guide = guide_axis(n.dodge = 3)) + 
    theme(legend.position = 'bottom') + 
    scale_color_manual +
    labs(color = 'Plot selection') +
    xlab('Site Type') + ylab(paste(taxa, 'Richness'))
}
```


```{r}
kruskal.test(distributed_sites_for_kruskal$bird_richness ~ distributed_sites_for_kruskal$site_type)

bird_richness_aov = aov(bird_richness ~ site_type, data = distributed_sites_for_kruskal)
bird_richness_tukey = TukeyHSD(bird_richness_aov)

bird_richness_groups = multcomp::cld(multcomp::glht(bird_richness_aov, linfct=multcomp::mcp(site_type="Tukey")))

bird_richness_groups_df = broom::tidy(bird_richness_groups)
bird_richness_groups_df
```

```{r}
(ggplot(distributed_sites, aes(x = site_type, y = bird_richness, color = plot_type)) + 
   geom_boxplot() + 
   geom_text(data = bird_richness_groups_df, mapping = aes(label = letters, y = 1), color = 'black') + bird_ylim) %>% 
  theme_common('Bird')
ggsave('output/birds.jpg')
```

```{r}
kruskal.test(distributed_sites_for_kruskal$insect_richness ~ distributed_sites_for_kruskal$site_type)

insect_richness_aov = aov(insect_richness ~ site_type, data = distributed_sites_for_kruskal)
insect_richness_tukey = TukeyHSD(insect_richness_aov)

insect_richness_groups = multcomp::cld(multcomp::glht(insect_richness_aov, linfct=multcomp::mcp(site_type="Tukey")))

insect_richness_groups_df = broom::tidy(insect_richness_groups)
insect_richness_groups_df
```

```{r}
(ggplot(distributed_sites, aes(x = site_type, y = insect_richness, color = plot_type)) + 
   geom_boxplot() + 
   geom_text(data = insect_richness_groups_df, mapping = aes(label = letters, y = 0.5), color = 'black') + insect_ylim) %>% 
  theme_common('Insect')
ggsave('output/insects.jpg')
```



```{r}
kruskal.test(distributed_sites_for_kruskal$plant_richness ~ distributed_sites_for_kruskal$site_type)

plant_richness_aov = aov(insect_richness ~ site_type, data = distributed_sites_for_kruskal)
plant_richness_tukey = TukeyHSD(plant_richness_aov)

plant_richness_groups = multcomp::cld(multcomp::glht(plant_richness_aov, linfct=multcomp::mcp(site_type="Tukey")))

plant_richness_groups_df = broom::tidy(plant_richness_groups)
plant_richness_groups_df
```

```{r}
(ggplot(distributed_sites, aes(x = site_type, y = plant_richness, color = plot_type)) + 
   geom_boxplot() + 
   geom_text(data = plant_richness_groups_df, mapping = aes(label = letters, y = 0.5), color = 'black') + plant_ylim) %>% 
  theme_common('Plant')
ggsave('output/plants.jpg')
```
